...
// }
// ---
__attribute((__always_inline__)) inline __m128 compute_distance(__m128 p1_r,
                                                                __m128 p1_g,
                                                                __m128 p1_b,
                                                                const pixel* p2,
                                                                int it) {
  // chargement des r, g et b en tant que float
  __m128 c_r = _mm_cvtepi32_ps(
      _mm_set_epi32(p2[it].r, p2[it + 1].r, p2[it + 2].r, p2[it + 3].r));
  __m128 c_g = _mm_cvtepi32_ps(
      _mm_set_epi32(p2[it].g, p2[it + 1].g, p2[it + 2].g, p2[it + 3].g));
  __m128 c_b = _mm_cvtepi32_ps(
      _mm_set_epi32(p2[it].b, p2[it + 1].b, p2[it + 2].b, p2[it + 3].b));
...
int find_best_cluster(const pixel image,
  ...
  // Chargement de r, g, et b en tant qu'entier
  ...
  for (int i = 0; i < N; i += 4) {
    ...
    const __m128 values =
        compute_distance(image_r, image_g, image_b, centers, i);
  ...
  // On cherche le minimum parmi les 4 minimums [min3, min2, min1, min0]
  idx = indices_array[0];
  min = values_array[0];
  for (int i = 1; i < 4; i++) {
...
// }
// ---
void compute_euclidean(float* distances,
                       const pixel* image,
                       int size,
                       pixel center) {
  int N = 4 * (size / 4);  // pour avoir un multiple de 4

  // Chargement de r, g et b
  ...
  // calcul des distances euclidiennes 4 par 4
  for (int i = 0; i < N; i += 4) {
    const __m128 values =
        compute_distance(center_r, center_g, center_b, image, i);
...
void kmeans_pp(pixel* image,
  ...
  // Calculate the euclidean distance between each pixel and the first center
  // randomly selected.
  ...
  for (int i = 1; i < num_clusters; i++) {
    ...
    // Generate a random number in [0, total_weight) and use it to select a new
    // center.
